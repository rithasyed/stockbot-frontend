...
export default function Home() {
  ...
  const fetchData = async (ticker: string, timeframe: string, emaPeriod: number, vwapPeriod: number, vwapStdDev: number) => {
    try {
      const response = await fetch(`http://192.168.1.216:5001/api/data/${ticker}/${timeframe}/${emaPeriod}/${vwapPeriod}/${vwapStdDev}`);
  ...
  const fetchWatchlist = async () => {
    try {
      const response = await fetch('http://192.168.1.216:5001/api/symbols');
